...
";
            ...
            Assert.Throws<CompilerException>(() =>
            {
                ...
                compiler.GetCompilation("ZpqrtBnk.ModelsBuilder.Generated", files, out trees);
            });
...
using Foo;
using ZpqrtBnk.ModelsBuilder.Tests;
...
";
            ...
            var compilation = CSharpCompilation.Create(
                "ZpqrtBnk.ModelsBuilder.RunTests",
                syntaxTrees: new[] { tree },
                references: refs,
                options: options);
            var model = compilation.GetSemanticModel(tree);
...
using ZpqrtBnk.ModelsBuilder.Tests; // imports ASCIIEncoding
...
using System.Text; // imports ASCIIEncoding
using ZpqrtBnk.ModelsBuilder.Tests; // imports ASCIIEncoding
namespace ZpqrtBnk.ModelsBuilder.Tests.Models // forces ZpqrtBnk.ModelsBuilder.Tests.ASCIIEncoding
{ }
";
...
";
            ...
            // implicit
            // finds ZpqrtBnk.ModelsBuilder.Umbraco
            // but we thought we'd find global Umbraco.Core.Models.IPublishedContent
            // so it is NOT ambiguous but will not compile because ZpqrtBnk.ModelsBuilder.Umbraco.Core... does not exist
            var lookup = model.LookupNamespacesAndTypes(pos, null, "Umbraco");
            Assert.AreEqual(1, lookup.Length);
            Assert.AreEqual("ZpqrtBnk.ModelsBuilder.Umbraco", lookup[0].ToDisplayString());
