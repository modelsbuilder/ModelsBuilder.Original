								...
								out string errMsg)
		{
			...
			try
			{
				...
				*/
				...
				//transform the retrieved filenames to generated.cs filesnames
				var generatedFileNames = genFiles.Select(s => $"{s.Key}.generated.cs").ToList();


				//remove files if model is removed from server
				//check if file is no longer in the generated list of files from the modelsbuilderapi response
				//if not in the response list, remove file from disk and project
				foreach (var file in Directory.GetFiles(path, "*.generated.cs"))
				{
					...
					if (!generatedFileNames.Contains(fileName))
					{
						...
						VisualStudioHelper.ClearExistingItem(vsitem, filename);
					}
				}

				//generate files
				//it's ok to overwrite current files, they wil get updated.
				//File.WriteAllText overwrites current existing file
